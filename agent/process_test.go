...
func TestFindCLINotFound(t *testing.T) {
	// Set PATH and HOME to prevent finding real CLI
...
func TestFindCLIInPath(t *testing.T) {
	...
	mustWriteFile(t, fakeClaude, []byte("#!/bin/sh\necho test"), 0755)
...
`
	...
	args := mustReadFile(t, filepath.Join(tmpDir, "args.txt"))
...
`
	...
	env := mustReadFile(t, filepath.Join(tmpDir, "env.txt"))
